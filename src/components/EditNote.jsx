...
function EditNote({ setShowEditNote, noteData}) {
  ...
  const handleAddNote = async (e) => {
    ...
    if (tagline.length < 50 && body.length < 500 && title.length < 100 && title.length > 0 && body.length > 0 && tagline.length > 0) {
      const res = await note.updateNote(noteData.$id, title, body, tagline)
      if (res === 1) {
        setPopupMsg({msg:"Updated Successfully",type:"success"})
        formRef.current.reset()
      }
      else{
        setPopupMsg({msg:"Error While Updating Note",type:"error"})
      }
    }
  }
  ...
  return (
    ...
    <div className='p-5 absolute flex z-50 justify-center  w-full h-full  bg-[#94949446]'>
      <div className='flex rounded-2xl gap-5 justify-center h-max flex-col p-2 w-1/3 bg-white'>
        <div className='flex w-full border-b py-2 px-2 justify-between  items-center'>
          ...
          <button onClick={() => setShowEditNote(false)} className='cursor-pointer border rounded-full px-5 py-2 flex justify-center items-center'>Close</button>
        </div>
        ...
        <div className='flex flex-col gap-2'>
          <form ref={formRef}>
            <div className='flex flex-col'>
              ...
              <input type="text" name='title' id='title' value={title} onChange={(e) => setTitle(e.target.value)} className='border rounded-md p-2' />
              <p className='text-gray-400 text-sm'>Max : 100</p>
              <p className='text-red-400 text-sm'>{titleError && titleError}</p>
            </div>
            <div className='flex flex-col'>
              <label htmlFor="body">Body</label>
              <input type="text" name='body' id='body' value={body} onChange={(e) => setBody(e.target.value)}        className='border rounded-md p-2' />
              <p className='text-gray-400 text-sm'>Max : 500</p>
              <p className='text-red-400 text-sm'>{bodyError && bodyError}</p>
            </div>
            <div className='flex flex-col'>
              <label htmlFor="tagline">Tagline</label>
              <input type="text" name='tagline' id='tagline' value={tagline} onChange={(e) => setTagline(e.target.value)} className='border rounded-md p-2' />
              <p className='text-gray-400 text-sm'>Max : 50</p>
              <p className='text-red-400 text-sm'>{taglineError && taglineError}</p>
            </div>
            <div className='flex justify-end'>
              <button onClick={handleAddNote} className='cursor-pointer border bg-green-500 rounded-xl px-8 py-3  flex justify-center items-center'>Updare</button>
            </div>
          </form>
        </div>
      </div>
